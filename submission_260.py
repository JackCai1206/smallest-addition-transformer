...
260-parameter transformer that learns 10-digit addition.
...
Architecture: 1-layer decoder, d_model=6, 2 heads, ffn=2, tied output head
  ...
  - FFN dim=2 (minimal) + weight decay 0.01 for fast convergence
...
def build_model():
    ...
    import os
    ckpt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "best_model_260.pt")
    ...
    metadata = {
        "name": "Spiral Addition Transformer (shared-XYZ, tied-proj, FFN2+WD)",
        "author": "Jack Cai",
        "params": n_params,
        ...
        "tricks": [
            ...
            "FFN dim=2 + weight decay 0.01 for fast convergence (~200 epochs)",
