...
273-parameter transformer that learns 10-digit addition via grokking.
...
Architecture: 1-layer decoder, d_model=6, 2 heads, ffn=3, tied output head
  ...
  - Shared XYZ: X[i], Y[i], Z[i] all use same digit_enc[i] (saves 60 params)
...
def build_pos_index_map():
    """Build mapping from sequence position to (source, index).
    source: 0=digit_enc, 1=z10_enc, 2=special_enc
    """
    sources = []
    indices = []
...
class SmallestAdditionTransformer(nn.Module):
    def __init__(self):
        ...
        # Shared XYZ positional encoding
        self.digit_enc = nn.Parameter(torch.zeros(10, POS_DIM))    # shared for X[i], Y[i], Z[i]
        ...
        # Index mapping for position -> encoding
...
def build_model():
    ...
    import os
    ckpt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "best_model_273.pt")
    ...
    metadata = {
        "name": "Spiral Addition Transformer (shared-XYZ, tied-proj, FFN3)",
        "author": "Jack Cai",
        "params": n_params,
        "architecture": "1L decoder, d=6 (3 tok + 3 pos), 2h, hd=3, ff=3, shared XYZ pos, tied output",
        "tricks": [
            ...
            "FFN dim=3 (minimal, works via grokking at ~300 epochs)",
