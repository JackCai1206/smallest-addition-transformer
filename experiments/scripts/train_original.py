"""Training loop for original (playful-water) variant: 1 layer, QK=pos, V=tok, with MLP."""
...
from model_original import SmallestAdditionTransformerOriginal
...
def train():
    ...
    model = SmallestAdditionTransformerOriginal().to(device)
    ...
    if use_wandb:
        wandb.init(
            ...
            config={
                "variant": "original_1layer_mlp_qkpos_vtok",
                "d_model": D_MODEL,
                "tok_dim": TOK_DIM,
                "pos_dim": POS_DIM,
                "n_pos": N_POS,
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_exact_acc > best_exact_acc:
            ...
            best_epoch = epoch
            patience_counter = 0
            torch.save(model.state_dict(), 'best_model_original.pt')
        else:
            patience_counter += 1
    ...
    model.load_state_dict(torch.load('best_model_original.pt', weights_only=True))
    val_loss, val_digit_acc, val_exact_acc, bucket_acc = evaluate(model, val_loader, device)
    bucket_str = " ".join(f"{nd}d:{acc:.3f}" for nd, acc in bucket_acc.items())
