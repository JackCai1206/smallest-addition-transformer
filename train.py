"""Training loop with evaluation for the smallest addition transformer."""
...
from model import SmallestAdditionTransformer
...
def evaluate(model, dataloader, device):
    ...
    Returns:
        avg_loss: teacher-forced loss on answer tokens
        digit_acc: per-digit accuracy (teacher-forced)
        exact_acc: exact-match accuracy (autoregressive)
        bucket_acc: dict {n_digits: exact_match_acc} (autoregressive)
    """
    model.eval()
    total_loss = 0.0
    total_correct = 0
    ...
    with torch.no_grad():
        for tokens, loss_mask, n_digits in dataloader:
            ...
            # --- Teacher-forced loss & digit accuracy ---
            ...
            prompt = tokens[:, :EQ_POS + 1]             # up to and including '='
            answer_target = tokens[:, EQ_POS + 1:]       # answer digits + EOS
            generated = model.generate(prompt)            # (B, ANSWER_LEN+1)

            exact = (generated == answer_target).all(dim=-1)  # (B,)
            total_exact += exact.sum().item()
            total_sequences += tokens.size(0)

            # Bucket by digit length
...
def train():
    ...
    # Model
    model = SmallestAdditionTransformer().to(device)
    ...
    if use_wandb:
        wandb.init(
            ...
            config={
                ...
                "pos_encoding": "hardcoded_sinusoidal",
            },
        )
