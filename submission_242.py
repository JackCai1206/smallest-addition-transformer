...
242-parameter transformer that learns 10-digit addition.

Architecture: 1-layer decoder, d_model=6, 2 heads, ffn=2, RMSNorm, tied output head
  - d_model=6 = tok_dim(3) + pos_dim(3), concatenated per position
  - Q,K from positional dims only; V from token dims only (split-head attention)
  - Shared XYZ positional encoding: X[i]=Y[i]=Z[i] saves 60 params
  - Tied output head: Linear(6->3) @ tok_emb.T instead of Linear(6->14)
  - RMSNorm instead of LayerNorm: saves 18 params (no bias in normalization)
  - FFN dim=2 (minimal) + weight decay 0.01 for grokking
  - Digits in LSB-first order for natural carry propagation
  - Spiral-initialized trainable embeddings: cos(2*pi*d/10), sin(2*pi*d/10), d/9
"""
...
# -- RMSNorm (saves 50% params vs LayerNorm: no bias, no mean centering) --
...
class SmallestAdditionTransformer(nn.Module):
    def __init__(self):
        ...
        # RMSNorm instead of LayerNorm — saves 18 params (3 norms * 6 bias params)
...
def build_model():
    ...
    import os
    ckpt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "best_model_242.pt")
    ...
    metadata = {
        "name": "Spiral Addition Transformer (RMSNorm, shared-XYZ, tied-proj, FFN2+WD)",
        "author": "Jack Cai",
        "params": n_params,
        ...
        "tricks": [
            ...
            "RMSNorm instead of LayerNorm: saves 18 params (no bias/mean in norms)",
