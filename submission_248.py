...
248-parameter transformer that learns 10-digit addition.

Architecture: 1-layer decoder, d_model=6, 2 heads, ffn=2, tied output head, shared LN
  - First 3 dims: token embedding (spiral-initialized, trainable)
  - Last 3 dims:  shared positional encoding (XYZ digit positions shared)
  - Q,K from positional dims only; V from token dims only
  - Tied output: Linear(6->3) @ tok_emb.T instead of Linear(6->14)
  - Shared XYZ: X[i]=Y[i]=Z[i] saves 60 params from positional encoding
  - Shared LayerNorm: ln1=ln2 saves 12 params (pre-attn and pre-FFN share norm)
  - FFN dim=2 (minimal) + weight decay 0.01 for grokking
...
class SmallestAdditionTransformer(nn.Module):
    def __init__(self):
        ...
        self.digit_enc = nn.Parameter(torch.zeros(10, POS_DIM))
        ...
        # Shared LN: ln1 is used for both pre-attention and pre-FFN (saves 12 params)
        self.ln1 = nn.LayerNorm(D_MODEL)
        # ln2 is omitted — ln1 is reused in forward()
    ...
    def _get_pos(self, T):
        pos = torch.zeros(T, POS_DIM, device=self.digit_enc.device)
        for t in range(T):
            src = self._pos_sources[t].item()
            idx = self._pos_indices[t].item()
            if src == 0:
                pos[t] = self.digit_enc[idx]
            elif src == 1:
    ...
    def forward(self, idx):
        ...
        x = x + self.ffn(self.ln1(x))  # shared: reuse ln1 for pre-FFN norm
        x = self.ln_f(x)
...
def build_model():
    ...
    import os
    ckpt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "best_model_248.pt")
    ...
    metadata = {
        "name": "Spiral Addition Transformer (shared-LN, shared-XYZ, tied-proj, FFN2+WD)",
        "author": "Jack Cai",
        "params": n_params,
        "architecture": "1L decoder, d=6 (3 tok + 3 pos), 2h, hd=3, ff=2, shared LN, shared XYZ pos, tied output",
        "tricks": [
            "Spiral-initialized trainable embeddings: (cos,sin,linear) for digits",
            "Split-head attention: Q,K from positional dims; V from token dims",
            "Shared XYZ positional encoding: X[i]=Y[i]=Z[i] saves 60 params",
            "Tied output head: Linear(6->3) @ tok_emb.T saves 66 params",
            "Shared LayerNorm: ln1=ln2 (pre-attn and pre-FFN share norm) saves 12 params",
            "FFN dim=2 + AdamW weight decay 0.01 for grokking",
